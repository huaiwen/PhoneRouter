/*
 *当这个activity启动时，在第101至104行的四个ArrayList数组里会存储根目录下的文件夹及文件信息，他们的大小size()-1表示了根目录下的文件数；
 *这时，如果想进入根目录的某个文件或文件夹，根据其位置，调用choose函数传入int代表第几个位置，如果这是个文件，则会打开（见第381行），如果这是个文件夹，
 *那么它里面的文件及文件夹列表信息会重新储存至刚才的四个数组中，同理数组的size()表示该目录下文件及文件夹数目
 *同理如果想再次往下访问，再调用choose()即可，以上过程不变。
 *如果想返回上一层，那么调用choose()传入-1即可
 */
...
import com.imudges.ldj.servse.SystemBroadCastReceiver;
import com.imudges.ldj.servse.ISDCardListener;
...
public class HDExplorerActivity extends Activity implements OnItemClickListener,ISDCardListener{
    ...
    ArrayList<String> filetime = new ArrayList<String>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.thismain);
        checkEnvironment();
        initMainUI();
        initData();
       ...
       /* choose(2);
        for(int j = 0;j < filename.size();j++){
        	System.out.println("@#-->"+filename.get(j)+filetime.get(j));
        }
        choose(-1);
        for(int j = 0;j < filename.size();j++){
        	System.out.println("@#-->"+filename.get(j)+filetime.get(j));
    ...
    private void open(File f ,boolean misAddToBackWardFiles){
        ...
        if(f.isFile()){
            ...
            for(File file:files){
                ...
                int icon = madapter.getFileIcon(f);
                if (icon == -1) {
					Drawable drawable = madapter.getApkIcon(file.getAbsolutePath());
					if (drawable != null) {
						filetype.add(drawable);
					} else {
						filetype.add("未知打开类型");
					}
				} else {
					filetype.add(Constant.getFileIcon(file));
				}
                filename.add(file.getName());
                filetime.add(transfiletime(file.lastModified()));
                filesize.add(transFileSize(file.length()));
   ...
   @Override
    ...
    public String transfiletime(long filetime) {
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
		String ftime = formatter.format(new Date(filetime));
		return ftime;
	}

	public String transFileSize(long filesize) {
		DecimalFormat df = new DecimalFormat("#.00");
		StringBuffer mstrbuf = new StringBuffer();
