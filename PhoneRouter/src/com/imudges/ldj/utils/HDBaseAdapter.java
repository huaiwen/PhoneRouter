...
import com.imudges.ldj.utils.Constant;
		...
		case VIEWMODE_LIST: {
			...
			if (f != null) {
				...
				if (f.isFile()) {
					...
					// System.out.println("---->"+this.getFileSize(f.length()));
					//filesize.add(this.getFileSize(f.length()));
				} else {
				...
				// System.out.println("---->"+this.getFileTime(f.lastModified()));
				//filetime.add(this.getFileTime(f.lastModified()));
			}
		}
			break;
		/*
		 * case VIEWMODE_ICON: { if(convertView==null) {
		 * convertView=LayoutInflater
		 * .from(mcontext).inflate(R.layout.item_gridview, null);
		 * mGridHolder=new GridHolder(); mGridHolder.mfileIcon=(ImageView)
		 * convertView.findViewById(R.id.gridview_fileicon);
		 * mGridHolder.mfileName=(TextView)
		 * convertView.findViewById(R.id.gridview_filename);
		 * convertView.setTag(mGridHolder); }else { mGridHolder=(GridHolder)
		 * convertView.getTag(); }
		 * 
		 * //update the holder File f = this.getItem(position); if(f != null){
		 * int icon = this.getFileIcon(f); if(icon == -1){ Drawable drawable =
		 * this.getApkIcon(f.getAbsolutePath()); if(drawable != null){
		 * mGridHolder.mfileIcon.setImageDrawable(drawable); } else{
		 * mGridHolder.mfileIcon.setImageResource(R.drawable.icon_file); }
		 * }else{ mGridHolder.mfileIcon.setImageResource(icon); }
		 * mGridHolder.mfileName.setText(f.getName());
		 * System.out.println("---->"+f.getName()); } } break;
		 */
		default:
			break;
		}
